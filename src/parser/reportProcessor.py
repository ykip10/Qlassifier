        ...
        for row in table.rows[1:]:
            ...
            for idx, cell in enumerate(cells):
                ...
                if mcq and not has_correct_ans_col and cell_highlighted(cell):
                    ...
                    mapping = {
                        idx: chr((idx - option_idx) + ord("a")).upper()
                        for idx in range(option_idx, len(cells[option_idx:-1]) + 1)
                    }
    ...
    def _parse_pdf_tables(self):
        ...
        for idx, df in enumerate(results):
            if idx == 0:  # skip mcq table
                continue
            try:
                new_df = pd.concat([df, comment_dfs[idx - 1]], axis=1)
            except IndexError as IE:
                raise IE(
                    "Too many tables scanned were classified as non-mcq."
