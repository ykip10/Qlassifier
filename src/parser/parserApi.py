...
def process_material(
...
) -> dict[str, Union[Tree, dict[str, Union[Tree, list[pd.DataFrame]]]]]:
    ...
    if type == "exam":
        ...
        raise ValueError(
            "Argument 'type' must be one of  ['exam', 'report, 'study_design']"
        )
    ...
    for subject in subjects:
        ...
        for file_name in dir_name.iterdir():
            if file_name.suffix not in [".docx", ".pdf"] or not any(
                str(year) in str(file_name) for year in years
            ):
                continue
