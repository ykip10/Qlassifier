...
from src.Qlassifier.prediction import (
...
def get_results(
    subjects: Sequence[str] | None = None, years: Sequence[int] | None = None
) -> list[list[Results]]:
    ...
    # Run all models
    model_aliases = ["instructor", "sentence-transformer", "tf-idf"]
    models = [
        INSTRUCTOR("hkunlp/instructor-large"),
        SentenceTransformer("intfloat/e5-base-v2"),
        TfidfVectorizer(),
    ]
    predictor_objs = [InstructPredictor, SentenceTransPredictor, TfIdfPredictor]

    all_res: dict[str, dict[str, list[Results]]] = []
    for model_alias, model, predictor_obj in zip(model_aliases, models, predictor_objs):
        ...
        for subj in subjects:
            ...
            for year in years:
                ...
                exam_names = (
                    [str(year)]
                    if not mc.has_two_exams(subj)
                    else [f"{year}_{i}" for i in [1, 2]]
                )
