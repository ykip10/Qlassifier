...
class TransformerPredictor(ABC):
    ...
    def classify(
        self, text: Sequence[str], return_cos: bool = False
    ) -> str | Sequence[float]:
    ...
    def run(
        self, exam: str | Path | pd.DataFrame, include_report: bool = False
    ) -> Results:
        ...
        if isinstance(exam, str) or isinstance(exam, Path):
            pred_df = prepare_exam_df(
                exam, subject=self.subject, load_report=include_report
            )
        ...
        pred_df["pred_topic"] = self.sd_df.loc[best_idxs, "label"].reset_index(
            drop=True
        )
        ...
        # Map to [0, 1]
        conf_col = (
            (conf_col - min(conf_col)) / (max(conf_col) - min(conf_col))
        ).tolist()
        pred_df["confidence"] = conf_col
        if include_report:
            report_input = (
                pred_df["comments"]
                if "comments" in pred_df.columns
                else [""] * len(qn_input)
            )
...
class InstructPredictor(TransformerPredictor):
    """Predictor for Instructor model."""

    def __init__(self, sd_path: str, subject: str, model: INSTRUCTOR | None = None):
        self.model = (
            model if model is not None else INSTRUCTOR("hkunlp/instructor-large")
        )
        super().__init__(sd_path, subject)

    def embed(self, questions: Sequence[str]) -> torch.Tensor:
        ...
        if "math" in self.subject:
            ...
            subject = "Mathematics"
        qn_instruct = (
            f"Represent this {std_str(subject)} question for semantic search: "
        )
    ...
    def _get_sd_emb(self):
        ...
        df = self.sd_df
        input = df["label"].str.cat(
            df["text"], sep="; "
        )  # Turns input into Topic; topic_description...
...
class SentenceTransPredictor(TransformerPredictor):
    """SentenceTransformer model ("intfloat/e5-base-v2")"""

    def __init__(
        self, sd_path: str, subject: str, model: SentenceTransformer | None = None
    ):
        self.model = (
            model if model is not None else SentenceTransformer("intfloat/e5-base-v2")
        )
        super().__init__(sd_path, subject)
...
class TfIdfPredictor:
    """Predictor built using TF-IDF."""

    def __init__(
        self, sd_path: str, subject: str, vectorizer: TfidfVectorizer | None = None
    ):
    ...
    def run(
        ...
        qns_ans_merged = (
            [
                (qn_desc + " " + ans_desc).strip()
                for qn_desc, ans_desc in zip(qns_desc, ans_desc)
            ]
            if include_report
            else [qn_desc.strip() for qn_desc in qns_desc]
        )
