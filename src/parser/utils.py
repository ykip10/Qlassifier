...
def crop(
    doc: pymupdf.Document, cr_coords: tuple[int], save_path: str | None = None
) -> pymupdf.Document:
...
def get_tables(path: str, pages: list[int] = None) -> dict[int, list[ExtractedTable]]:
    ...
    """
    pdf = PDF(path, pages=pages, detect_rotation=False, pdf_text_extraction=True)
...
def convert_extracted_tables(
    ...
    def df_is_mcq(df: pd.DataFrame) -> bool:
        ...
        has_percentage_cols = (
            any("%" in str(col) for col in columns) if str_cols else False
        )

        has_marks_col = (
            any(re.match("(M|m)arks?", str(col)) for col in columns)
            if str_cols
            else False
        )
        ...
        all_digit_cols = all(
            str(col).strip().isdigit() for col in columns if col is not None
        )
...
def process_tables(
    ...
    if mcq_dfs:
        ...
        for idx, df in enumerate(mcq_dfs):
            ...
            correct_ans_col = pd.DataFrame({"correct_answer": [np.nan] * df.shape[0]})
    ...
    for idx, df in enumerate(sa_dfs):
        ...
        if not any(
            bool(re.match("(M|m)ark", string=col))
            for col in new_cols
            if col is not None
        ):
        ...
        if any("average" in col for col in lower):
            ...
            lower[-1] = (
                "average"  # Extract float from something like "average\n{float}"
            )
            df.columns = lower
...
def standardise_mcq_df(
    df: pd.DataFrame, edit_correct_ans_col: bool = True
) -> pd.DataFrame:
    ...
    if edit_correct_ans_col:
        ...
        reordered_cols = (
            list(cols[:option_idx]) + [cols[-1]] + list(cols[option_idx:-1])
        )
